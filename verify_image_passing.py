...
class TestImagePassing(unittest.TestCase):
    def setUp(self):
        ...
        self.mock_context.get_config.return_value = {}

        # 实例化被测试的插件
        self.plugin = ContextEnhancerV2(self.mock_context)
        
        # 模拟配置
        self.plugin.config = self.plugin.get_default_config()
        self.plugin.config["启用群组"] = [] # 对所有群生效
    ...
    def test_image_url_passing(self):
        async def run_test():
...
if __name__ == "__main__":
    # 为了在非 AstrBot 环境下运行，需要 mock 一些不存在的模块
    import sys
